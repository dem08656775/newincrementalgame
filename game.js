  ...
  computed: {
    tweetLink() {
      ...
      if (this.player.tweeting.includes("money")) {
        tweetText += `ポイント:${this.player.money}(${this.player.money.toExponential().replace("+", "%2B")})%0A`
      }
      if (this.player.tweeting.includes("darkmoney")) {
        tweetText += `裏ポイント:${this.player.darkmoney}(${this.player.darkmoney.toExponential().replace("+", "%2B")})%0A`
      }
      ...
      return `https://twitter.com/intent/tweet?text=${tweetText}&url=${tweetUrl}&hashtags=${tweetHashtag}`
    },
  },
  methods: {
    configshowmult() {
    ...
    load(world) {
      const saveData = this.players[world]
      this.world = world
    ...
    resetLevelBorder() {
      const inChallenge1 = this.player.onchallenge && this.player.challenges.includes(0)
      return new Decimal(10).pow(inChallenge1 ? 24 : 18)
    },
    resetRankborder() {
